...
class image_stitch(object):
    def __init__(self):
        ...
        self.camera23 = None
        self.camera24 = None
        self.mask = True       #use False to stitch color images...use True to stitch masks
    ...
    def stitch_images(self,homography_list,img_array):
        ...
        if self.mask and len(img_array[0].shape) != 3:
            img_array = self.flip_mask(img_array)
        ...
        for i, homography in enumerate(homography_list):
            ...
            try:
                ...
                if i == 0:
                    ...
                    im_copy[b_y:b.shape[0] + b_y,
                    b_x:b.shape[1] + b_x] = b  # check sizing using blank image to get the best image size
            ...
            new_pts_warped = np.array(new_pts_warped)[0:,
                             0:2]  # get the first 2 columns since the third column is just ones
    ...
    def flip_mask(self, img_array):
        new_array = []
        for img in img_array:
            img = cv2.bitwise_not(img)
            new_img = np.zeros((np.array(img).shape[0], np.array(img).shape[1], 3), dtype=np.uint8)
            new_img[:, :, 0] = img  # same value in each channel
            new_img[:, :, 1] = img
            new_img[:, :, 2] = img
    ...
    def image_analyis(self):
        ...
        # homography_list = homography_list[:10]
        # # print(len(homography_list))
        ...
        while not rospy.is_shutdown():
            ...
            if len(cond) == 4:
                ...
                for (imga, imgb) in stereo_cams:
                    ...
                    # filt_color = cv2.applyColorMap(filteredImg, cv2.COLORMAP_SUMMER)
                    mask_imgs_stitch.append(mask_temp)


                # print(len(mask_imgs_stitch),len(real_imgs_stitch))
                print("starting stitch")
                # mask_imgs_stitch = self.flip_mask(mask_imgs_stitch)
                final_stitch = self.stitch_images(homography_list, mask_imgs_stitch)
                ...
                #
                cv2.imwrite("save_imgs/final_mask_stitch.png", final_stitch)
                ...
                cv2.imwrite("save_imgs/mask5.png", mask_imgs_stitch[4])
                cv2.imwrite("save_imgs/mask6.png", mask_imgs_stitch[5])
                cv2.imwrite("save_imgs/mask7.png", mask_imgs_stitch[6])
                cv2.imwrite("save_imgs/mask8.png", mask_imgs_stitch[7])
                cv2.imwrite("save_imgs/mask9.png", mask_imgs_stitch[8])
                cv2.imwrite("save_imgs/mask10.png", mask_imgs_stitch[9])
                cv2.imwrite("save_imgs/mask11.png", mask_imgs_stitch[10])
                cv2.imwrite("save_imgs/mask12.png", mask_imgs_stitch[11])



                final_img = final_stitch  #final image to be shown
